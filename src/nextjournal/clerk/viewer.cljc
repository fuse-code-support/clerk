...
(defn assign-closing-parens
  ...
  ([parens-acc node]
   (let [value (value node)
     ...
     (cond-> node
       (and closing (not defer-closing?)) (update-in [:nextjournal/viewer :closing-paren] str parens-acc)
       (and closing defer-closing?) (update :nextjournal/viewer dissoc :closing-paren)
       non-leaf? (update :nextjournal/value
                         (fn [xs]
                           (into []
                                 (map-indexed (fn [i x]
                                                (assign-closing-parens (if (and defer-closing? (= (dec (count xs)) i))
                                                                         (str closing parens-acc)
                                                                         "")
                                                                       x)))
                                 xs)))))))
